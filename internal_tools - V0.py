...
def components_retrival(queried_components: List[Dict], queried_model, models_dict):
    ...
    # Special handling for demand increase scenario
    if any(comp['component_name'] == 'Demand' for comp in queried_components):
        feedback = "Analyzing multiple demand increase scenarios:\n\n"
        percentages = [5, 7, 10, 12, 15]  # We can analyze multiple scenarios
        
        for percentage in percentages:
            result = analyze_demand_change(model, percentage)
            feedback += format_demand_analysis(result) + "\n" + "="*50 + "\n\n"
        
        return feedback
    
    # Original components_retrival logic for other cases
