...
import sys
import time
from db_schema import store_scenario_results, get_scenario_results
...
def get_all_demand_indexes(model):
    """Helper function to get all demand indexes"""
    return [(s, p) for s in model.Stores for p in model.Products]


def analyze_demand_change(model, percentage):
    """Analyze impact of demand change by given percentage"""
    all_demand_indexes = get_all_demand_indexes(model)
    
    # Store original values and solution
    original_values = {idx: model.Demand[idx].value for idx in all_demand_indexes}
    original_solution = model.solution if hasattr(model, 'solution') else None
    
    # Modify demand values
    for idx in all_demand_indexes:
        model.Demand[idx].value = original_values[idx] * (1 + percentage/100)
    ...
    result = {
        ...
        'new_solution': model.solution if hasattr(model, 'solution') else None,
        'percentage': percentage
    }
    
    # Restore original values
    for idx in all_demand_indexes:
        model.Demand[idx].value = original_values[idx]
        
    return result


def format_demand_analysis(result):
    """Format the demand analysis results into readable feedback"""
    feedback = f"Impact of {result['percentage']}% Demand Increase:\n\n"
    
    if not result['feasible']:
        feedback += f"\nWarning: The model becomes infeasible with {result['percentage']}% higher demand.\n"
        feedback += "This suggests that current production or transportation capacity cannot handle the increased demand.\n"
        feedback += "Consider reviewing capacity constraints or exploring capacity expansion options."
        return feedback
    
    if result['original_solution'] and result['new_solution']:
        orig_costs = result['original_solution']['costs']
        new_costs = result['new_solution']['costs']
        
        # Original costs
        feedback += "Original Costs:\n"
        feedback += f"- Total Production Cost: ${orig_costs['total_production_cost']:,.2f}\n"
        feedback += f"- Total Transportation Cost: ${orig_costs['total_transportation_cost']:,.2f}\n"
        feedback += f"- Total Assembly-to-Store Cost: ${orig_costs['total_assembly_to_store_cost']:,.2f}\n"
        orig_total = (orig_costs['total_production_cost'] + 
                     orig_costs['total_transportation_cost'] + 
                     orig_costs['total_assembly_to_store_cost'])
        feedback += f"- Total Cost: ${orig_total:,.2f}\n\n"
        
        # New costs
        feedback += f"New Costs with {result['percentage']}% Higher Demand:\n"
        feedback += f"- Total Production Cost: ${new_costs['total_production_cost']:,.2f}\n"
        feedback += f"- Total Transportation Cost: ${new_costs['total_transportation_cost']:,.2f}\n"
        feedback += f"- Total Assembly-to-Store Cost: ${new_costs['total_assembly_to_store_cost']:,.2f}\n"
        new_total = (new_costs['total_production_cost'] + 
                    new_costs['total_transportation_cost'] + 
                    new_costs['total_assembly_to_store_cost'])
        feedback += f"- Total Cost: ${new_total:,.2f}\n\n"
        
        # Calculate percentage changes
        prod_change = ((new_costs['total_production_cost'] - orig_costs['total_production_cost']) / 
                      orig_costs['total_production_cost'] * 100)
        trans_change = ((new_costs['total_transportation_cost'] - orig_costs['total_transportation_cost']) / 
                       orig_costs['total_transportation_cost'] * 100)
        assembly_change = ((new_costs['total_assembly_to_store_cost'] - orig_costs['total_assembly_to_store_cost']) / 
                          orig_costs['total_assembly_to_store_cost'] * 100)
        total_change = ((new_total - orig_total) / orig_total * 100)
        
        feedback += "Percentage Changes:\n"
        feedback += f"- Production Cost Change: {prod_change:.1f}%\n"
        feedback += f"- Transportation Cost Change: {trans_change:.1f}%\n"
        feedback += f"- Assembly-to-Store Cost Change: {assembly_change:.1f}%\n"
        feedback += f"- Total Cost Change: {total_change:.1f}%\n"
...
def components_retrival(queried_components: List[Dict], queried_model, models_dict):
    ...
    # Check for what-if analysis requests
    what_if_components = [comp for comp in queried_components if comp['component_name'] in 
                         ['Demand', 'Capacity', 'ProductionCost', 'TransportationCost']]
    
    if what_if_components:
        scenario_params = {}
        
        # Process demand changes
        demand_changes = {(comp['component_indexes'][0], comp['component_indexes'][1]): float(comp.get('delta', 10))
                         for comp in what_if_components if comp['component_name'] == 'Demand'}
        if demand_changes:
            scenario_params['demand_changes'] = demand_changes
            
        # Process capacity changes
        capacity_changes = {(comp['component_indexes'][0], comp['component_indexes'][1]): float(comp.get('delta', 10))
                          for comp in what_if_components if comp['component_name'] == 'Capacity'}
        if capacity_changes:
            scenario_params['capacity_changes'] = capacity_changes
            
        # Process cost changes
        cost_changes = {}
        for comp in what_if_components:
            if comp['component_name'] == 'ProductionCost':
                cost_changes['production'] = float(comp.get('delta', 10))
            elif comp['component_name'] == 'TransportationCost':
                cost_changes['transportation'] = float(comp.get('delta', 10))
        if cost_changes:
            scenario_params['cost_changes'] = cost_changes
            
        # Run scenario analysis
        if scenario_params:
            scenario_results = analyze_supply_chain_scenario(model, scenario_params)
            return format_scenario_results(scenario_results)
    
    # If not a what-if analysis request, proceed with original components_retrival logic
    solution = model.solution if hasattr(model, 'solution') else None
    feedback = "Based on the optimization results:\n\n"
    
    # 1. Total Cost and Profit Information
    if solution and 'costs' in solution:
        costs = solution['costs']
        feedback += "Financial Summary:\n"
        feedback += f"- Total Revenue: ${costs['total_revenue']:,.2f}\n"
        feedback += f"- Total Production Cost: ${costs['total_production_cost']:,.2f}\n"
        feedback += f"- Total Transportation Cost: ${costs['total_transportation_cost']:,.2f}\n"
        feedback += f"- Total Assembly-to-Store Cost: ${costs['total_assembly_to_store_cost']:,.2f}\n"
        feedback += f"- Net Profit: ${costs['net_profit']:,.2f}\n"
        total_cost = costs['total_production_cost'] + costs['total_transportation_cost'] + costs['total_assembly_to_store_cost']
        feedback += f"- Total Cost: ${total_cost:,.2f}\n\n"
    
    # 2. Production Plan
    if solution and 'production' in solution:
        feedback += "Optimal Production Plan:\n"
        for (fab, prod), quantity in solution['production'].items():
            if quantity > 0:
                feedback += f"- {fab} produces {quantity:,.0f} units of {prod}\n"
        feedback += "\n"
    
    # 3. Transportation Plan
    if solution and 'transportation' in solution:
        feedback += "Transportation Plan:\n"
        for (fab, assembly, prod), quantity in solution['transportation'].items():
            if quantity > 0:
                feedback += f"- Ship {quantity:,.0f} units of {prod} from {fab} to {assembly}\n"
        feedback += "\n"
    
    # 4. Assembly to Store Distribution
    if solution and 'assembly_to_store' in solution:
        feedback += "Store Distribution Plan:\n"
        for (assembly, store, prod), quantity in solution['assembly_to_store'].items():
            if quantity > 0:
                feedback += f"- Deliver {quantity:,.0f} units of {prod} from {assembly} to {store}\n"
        feedback += "\n"
    
    # If no solution is available, try to get values directly from the model
    if not solution:
        feedback += "Warning: Could not find stored solution. Attempting to retrieve values directly:\n\n"
        
        # Process queried components
        for component in queried_components:
            ...
            try:
                if isinstance(component_indexes, tuple):
                    # Multiple components with tuple index
                    if slice(None) in component_indexes:
                        feedback += f"{component_name.title()} Values:\n"
                        for model_component_i in model_component[component_indexes]:
                            idx = model_component_i.index()
                            val = pe.value(model_component_i) if hasattr(model_component_i, 'value') else model_component_i()
                            feedback += f"  At {str(idx)}: {val:,.2f}\n"
                    else:
                        # Single component with tuple index
                        val = pe.value(model_component[component_indexes]) if hasattr(model_component[component_indexes], 'value') else model_component[component_indexes]()
                        feedback += f"{component_name.title()} at {str(component_indexes)}: {val:,.2f}\n"
                
                elif isinstance(component_indexes, slice):
                    # Multiple components with slice
                    feedback += f"{component_name.title()} Values:\n"
                    for model_component_i in model_component[component_indexes]:
                        idx = model_component_i.index()
                        val = pe.value(model_component_i) if hasattr(model_component_i, 'value') else model_component_i()
                ...
                elif isinstance(component_indexes, (int, str)) or component_indexes is None:
                    # Single component
                    idx_str = f" at {str(component_indexes)}" if component_indexes is not None else ""
                    val = pe.value(model_component[component_indexes]) if hasattr(model_component[component_indexes], 'value') else model_component[component_indexes]()
                    feedback += f"{component_name.title()}{idx_str}: {val:,.2f}\n"
                
                feedback += "\n"
                
            except Exception as e:
                feedback += f"Error retrieving {component_name}: {str(e)}\n\n"
...
def analyze_supply_chain_scenario(model, scenario_params, scenario_name=None, description=None):
    """
    Comprehensive scenario analysis for supply chain what-if analysis.
    
    Args:
        model: The Pyomo model instance
        scenario_params: Dictionary containing scenario parameters:
            - demand_changes: Dict of {(store, product): percentage_change}
            - capacity_changes: Dict of {(facility, product): percentage_change}
            - cost_changes: Dict of {cost_type: percentage_change}
            - disruption_params: Dict of facility disruption parameters
        scenario_name: Optional name for the scenario
        description: Optional description of the scenario
    """
    # Generate default scenario name if not provided
    if scenario_name is None:
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        changes = []
        if 'demand_changes' in scenario_params:
            changes.append('demand')
        if 'capacity_changes' in scenario_params:
            changes.append('capacity')
        if 'cost_changes' in scenario_params:
            changes.append('cost')
        if 'disruption_params' in scenario_params:
            changes.append('disruption')
        scenario_name = f"scenario_{'-'.join(changes)}_{timestamp}"
    
    # Generate default description if not provided
    if description is None:
        description = "Scenario analysis with "
        if 'demand_changes' in scenario_params:
            description += f"demand changes ({len(scenario_params['demand_changes'])} changes), "
        if 'capacity_changes' in scenario_params:
            description += f"capacity changes ({len(scenario_params['capacity_changes'])} changes), "
        if 'cost_changes' in scenario_params:
            description += f"cost changes ({len(scenario_params['cost_changes'])} changes), "
        if 'disruption_params' in scenario_params:
            description += f"disruption parameters ({len(scenario_params['disruption_params'])} facilities), "
        description = description.rstrip(", ")
    
    # Store original values
    original_state = store_model_state(model)
    
    try:
        # Apply scenario changes
        if 'demand_changes' in scenario_params:
            apply_demand_changes(model, scenario_params['demand_changes'])
        
        if 'capacity_changes' in scenario_params:
            apply_capacity_changes(model, scenario_params['capacity_changes'])
            
        if 'cost_changes' in scenario_params:
            apply_cost_changes(model, scenario_params['cost_changes'])
            
        if 'disruption_params' in scenario_params:
            apply_disruption_scenario(model, scenario_params['disruption_params'])
        ...
        # Analyze results
        scenario_results = analyze_scenario_results(model, results, scenario_params)
        
        # Store results in database
        scenario_id = store_scenario_results(
...
def store_model_state(model):
    """Store the current state of all model parameters and variables"""
    state = {}
    
    # Store parameter values
    for param_name, param in model.component_map(pe.Param).items():
        if param.is_indexed():
            state[param_name] = {idx: param[idx].value for idx in param.index_set()}
        else:
            state[param_name] = param.value
            
    # Store variable values and fixed status
    for var_name, var in model.component_map(pe.Var).items():
        if var.is_indexed():
            state[var_name] = {idx: (var[idx].value, var[idx].fixed) for idx in var.index_set()}
        else:
            state[var_name] = (var.value, var.fixed)
            
    return state

def restore_model_state(model, state):
    """Restore model to its original state"""
    for comp_name, values in state.items():
        component = getattr(model, comp_name)
        
        if isinstance(values, dict):  # Indexed component
            for idx, value in values.items():
                if isinstance(value, tuple):  # Variable with value and fixed status
                    component[idx].value = value[0]
                    if value[1]:  # If was fixed
                        component[idx].fix(value[0])
                    else:
                        component[idx].unfix()
                else:  # Parameter
                    component[idx].value = value
        else:  # Scalar component
            if isinstance(values, tuple):  # Variable
                component.value = values[0]
                if values[1]:  # If was fixed
                    component.fix(values[0])
                else:
                    component.unfix()
            else:  # Parameter
                component.value = values

def apply_demand_changes(model, demand_changes):
    """Apply demand changes to the model"""
    for (store, product), change in demand_changes.items():
        if hasattr(model, 'Demand'):
            current_demand = model.Demand[store, product].value
            new_demand = current_demand * (1 + change/100)
            model.Demand[store, product].value = new_demand

def apply_capacity_changes(model, capacity_changes):
    """Apply capacity changes to the model"""
    for (facility, product), change in capacity_changes.items():
        if hasattr(model, 'Capacity'):
            current_capacity = model.Capacity[facility, product].value
            new_capacity = current_capacity * (1 + change/100)
            model.Capacity[facility, product].value = new_capacity

def apply_cost_changes(model, cost_changes):
    """Apply cost changes to various cost parameters"""
    for cost_type, change in cost_changes.items():
        if cost_type == 'production' and hasattr(model, 'ProductionCost'):
            for idx in model.ProductionCost.index_set():
                current_cost = model.ProductionCost[idx].value
                new_cost = current_cost * (1 + change/100)
                model.ProductionCost[idx].value = new_cost
        elif cost_type == 'transportation' and hasattr(model, 'TransportationCost'):
            for idx in model.TransportationCost.index_set():
                current_cost = model.TransportationCost[idx].value
                new_cost = current_cost * (1 + change/100)
                model.TransportationCost[idx].value = new_cost

def apply_disruption_scenario(model, disruption_params):
    """Apply supply chain disruption scenarios"""
    for facility, params in disruption_params.items():
        if 'capacity_reduction' in params:
            reduction = params['capacity_reduction']
            if hasattr(model, 'Capacity'):
                for product in model.Products:
                    if (facility, product) in model.Capacity.index_set():
                        current_capacity = model.Capacity[facility, product].value
                        new_capacity = current_capacity * (1 - reduction/100)
                        model.Capacity[facility, product].value = new_capacity

def analyze_scenario_results(model, results, scenario_params):
    ...
    if scenario_results['feasible']:
        # Calculate costs
        if hasattr(model, 'TotalCost'):
            scenario_results['costs']['total'] = pe.value(model.TotalCost)
        
        # Calculate production quantities
        if hasattr(model, 'Production'):
            for idx in model.Production.index_set():
                if pe.value(model.Production[idx]) > 0:
                    scenario_results['production'][idx] = pe.value(model.Production[idx])
        
        # Calculate transportation flows
        if hasattr(model, 'Transportation'):
            for idx in model.Transportation.index_set():
                if pe.value(model.Transportation[idx]) > 0:
                    scenario_results['transportation'][idx] = pe.value(model.Transportation[idx])
        
        # Calculate capacity utilization
        if hasattr(model, 'Production') and hasattr(model, 'Capacity'):
            for (facility, product) in model.Capacity.index_set():
                if model.Capacity[facility, product].value > 0:
                    utilization = (pe.value(model.Production[facility, product]) / 
                                 model.Capacity[facility, product].value * 100)
                    scenario_results['capacity_utilization'][(facility, product)] = utilization
        
        # Calculate service level (demand fulfillment)
        if hasattr(model, 'Demand') and hasattr(model, 'SalesQuantity'):
            for (store, product) in model.Demand.index_set():
                if model.Demand[store, product].value > 0:
                    service_level = (pe.value(model.SalesQuantity[store, product]) / 
                                   model.Demand[store, product].value * 100)
                    scenario_results['service_level'][(store, product)] = service_level
    
    return scenario_results

def format_scenario_results(scenario_results):
    """Format scenario analysis results into readable feedback"""
    feedback = "Scenario Analysis Results:\n\n"
    
    # Scenario description
    feedback += "Scenario Parameters:\n"
    for param_type, changes in scenario_results['scenario_params'].items():
        feedback += f"- {param_type.replace('_', ' ').title()}:\n"
        for key, value in changes.items():
            feedback += f"  * {key}: {value}%\n"
    feedback += "\n"
    
    if not scenario_results['feasible']:
        feedback += "WARNING: The scenario is INFEASIBLE with the given parameters.\n"
        feedback += "This suggests that the supply chain cannot operate under these conditions.\n"
        feedback += "Consider reviewing the scenario parameters or supply chain constraints.\n"
        return feedback
    
    # Cost Analysis
    if scenario_results['costs']:
        feedback += "Cost Analysis:\n"
        for cost_type, value in scenario_results['costs'].items():
            feedback += f"- {cost_type.replace('_', ' ').title()}: ${value:,.2f}\n"
        feedback += "\n"
    
    # Capacity Utilization
    if scenario_results['capacity_utilization']:
        feedback += "Capacity Utilization:\n"
        for (facility, product), utilization in scenario_results['capacity_utilization'].items():
            feedback += f"- {facility} ({product}): {utilization:.1f}%\n"
        feedback += "\n"
    
    # Service Level
    if scenario_results['service_level']:
        feedback += "Service Level Analysis:\n"
        for (store, product), service_level in scenario_results['service_level'].items():
            feedback += f"- {store} ({product}): {service_level:.1f}%\n"
        feedback += "\n"
