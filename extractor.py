...
def initial_loading(file, data, is_uploaded=True):
    if is_uploaded:
        ...
        model = uploaded_model.create_model(data)
...
def create_model(data):
    """
    Create a supply chain Pyomo model from a data dictionary.
    The data dictionary should contain sets and parameters for products, stores, fabs, assemblies, and all relevant costs and capacities.
    """
    required_keys = ['Products', 'Stores', 'Fabs', 'Assemblies', 'Demand', 'Revenue', 'FabCapacity', 'FabCost', 'TransCost', 'TransCapacity']
    for key in required_keys:
        if key not in data:
            raise ValueError(f"Missing required key in data: {key}")
    # ... rest of your code ...

    model.Demand = Param(model.Stores, model.Products, initialize={k: int(v) for k, v in data['Demand'].items()}, default=0)
    # Repeat for other parameters as needed

    model.Prod = Var(model.Fabs, model.Products, domain=NonNegativeReals, doc="Production quantity of each product at each fab")
    model.Trans = Var(model.Fabs, model.Assemblies, model.Products, domain=NonNegativeReals, doc="Transported quantity from fab to assembly")
    # And so on for constraints

    model.Profit = Objective(rule=profit_rule, sense=maximize, doc="Maximize total profit (revenue - production cost - transportation cost)")


def components_retrival(queried_components, queried_model='model_1', models_dict=None):
    """
    Retrieve components from the model based on the query.
    """
    if models_dict is None or queried_model not in models_dict:
        print("Error: Invalid model reference")
        return
        
    model = models_dict[queried_model]['model class']
    ...
    if hasattr(model, 'solution'):
        ...
        print("\nFinancial Breakdown:")
        print(f"Total Revenue: ${solution['costs']['total_revenue']:,.2f}")
        print(f"Total Production Cost: ${solution['costs']['total_production_cost']:,.2f}")
        print(f"Total Transportation Cost: ${solution['costs']['total_transportation_cost']:,.2f}")
        print(f"Total Assembly-to-Store Cost: ${solution['costs']['total_assembly_to_store_cost']:,.2f}")
        print(f"Net Profit: ${solution['costs']['net_profit']:,.2f}")
        total_cost = (solution['costs']['total_production_cost'] + 
                     solution['costs']['total_transportation_cost'] + 
                     solution['costs']['total_assembly_to_store_cost'])
        print(f"\nTotal Cost: ${total_cost:,.2f}")
        
        print("\nOptimal Production Plan:")
        for (f, p), qty in solution['production'].items():
            if qty > 0:
                print(f"Production at {f} for {p}: {qty:,.0f} units")
                
        print("\nOptimal Transportation Plan:")
        for (f, a, p), qty in solution['transportation'].items():
            if qty > 0:
                print(f"Transport from {f} to {a} for {p}: {qty:,.0f} units")
                
        print("\nOptimal Assembly-to-Store Plan:")
        for (a, s, p), qty in solution['assembly_to_store'].items():
            if qty > 0:
                print(f"Transport from {a} to {s} for {p}: {qty:,.0f} units")
    else:
        # Fall back to old method if solution not stored in new format
        try:
            ...
            print(f"Error displaying cost components: {str(e)}")
        
        for queried_component in queried_components:
            try:
                component_name = queried_component['component_name']
                ...
                if hasattr(component, 'get_values'):
                    try:
                        ...
                        if values:
                            ...
                            for idx, val in values.items():
                                if val != 0:
                                    print(f"{component_name}[{idx}] = {val:,.0f}")
                    except Exception as e:
                        print(f"Error getting values for {component_name}: {str(e)}")
                elif hasattr(component, 'extract_values'):
                    try:
                        ...
                        print(f"Error extracting values for {component_name}: {str(e)}")
                elif hasattr(component, 'keys'):
                    print(f"\n{component_name} Values:")
                    for idx in component.keys():
                        try:
                            ...
                            print(f"Error retrieving {component_name}[{idx}]: {str(e)}")
                else:
                    val = None
                    if hasattr(component, 'value'):
                        ...
                        val = component
                    print(f"\n{component_name} = {val:,.0f}")
            except Exception as e:
                print(f"Error processing component {queried_component}: {str(e)}")
                continue